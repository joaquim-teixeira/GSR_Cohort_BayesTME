...
analysis_lda_patient=function (f_coords, f_btme,f_btme_2, f_lda_slice,f_lda_patient, f_clust,indvals)
{
  dat=read.table(f_coords)
  cds=get_coords_cdata(dat[1,-1])
  ...
  probs_lda=pivot_longer(probs_lda,cols=starts_with('cluster'))
  probs_btme=pivot_longer(probs_btme,cols=starts_with('cluster'))
  probs_lda$type='3-LDA_slice'
  probs_btme$type='1-BayesTME_1e4'
  btme_2=get_nc_list(f_btme_2)
  probs_btme_2=data.frame (cds, t(btme_2$cell_prob_trace[-1,]))
  names (probs_btme_2)=c('x','y',paste ('cluster_',1:6,sep=''))

  probs_btme_2=pivot_longer(probs_btme_2,cols=starts_with('cluster'))
  probs_btme_2$type='2-BayesTME_1e6'
  ...
  probs_hclust$type='5-Hierarchical'
  ...
  probs_lda_patient$type='4-LDA_patient'
  probs=rbind (probs_lda, probs_btme,probs_btme_2,probs_hclust,probs_lda_patient)
...
flb=list.files('/Users/joaquimteixeira/GSR_22_Tansey_Banerjee/teixeira_code/deconvolution_results_1000_sims',full.names = T)[1:6]
flb_2=list.files('/Users/joaquimteixeira/GSR_22_Tansey_Banerjee/teixeira_code/deconvolution_results_1000_sims_lambda_10e5/',full.names = T)[1:6]
...
We look at a comparison between a few ST analyses on the HER2-positive breast tumors provided in Andersson et al. (2021). The data consists of 7 patients each with 3-6 slices. So far, this analysis is only on patient 'A'. The following analyses were performed:
...
2. Latent Direchlet Allocation applied (LDA) on each slice, based on sklearn.decomposition.LatentDirichletAllocation. We enforce 6 cell types.

3. Latent Direchlet Allocation applied on each patient. We enforce 6 cell types.

4. The baseline analysis performed by Andersson et al. (2021). Nearest neighbors/hierarchical clustering  is performed on gene expression at the patient+cohort level. This was used to set the number of cell types in each slice. 
...
for (i in 1:6)
{
  vals[[i]]=analysis_lda_patient(flc[i],flb[i],flb_2[i],fll[i],fllp[i],flh[i],inds[i,])
}
```
